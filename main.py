...
def create_sahi_augmented_dataset(data_yaml_path, output_dir, slice_size=640, overlap_ratio=0.2):
    """
    Create a SAHI-augmented dataset from the original dataset.
...
def evaluate_with_sahi(model_path, data_yaml_path, slice_size=640, overlap_ratio=0.2):
    ...
    for img_path in tqdm(images, desc="Evaluating with SAHI"):
        ...
        sliced_prediction.export_visuals(export_dir="demo_data/", file_name=img_name, rect_th=2)
...
def main():
    ...
    if not args.skip_slicing:
        data_yaml_path = create_sahi_augmented_dataset(
            args.data, 
            args.output, 
            args.slice_size, 
            args.overlap_ratio
    ...
    evaluate_with_sahi(f"C://Users/rocha/Documents/NACRE/sahi_yolo/yolov8{args.model_size}/weights/best.pt", args.data, args.slice_size, args.overlap_ratio)
    print("Done!")
